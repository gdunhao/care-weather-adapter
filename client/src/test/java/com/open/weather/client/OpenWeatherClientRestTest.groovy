...
class OpenWeatherClientRestTest extends Specification {
  ...
  def "Should return weather response"() {
    given:
    whenHttp(stubServer).match(endsWithUri(WEATHER_URL))
      .then(ok(), resourceContent("_files/unit/weather/weather.json"))

    when:
    WeatherRS response = openWeatherClient.fetchWeather(2950159)
    ...
    verifyHttp(stubServer).once(method(Method.GET), endsWithUri(WEATHER_URL))
  }
}